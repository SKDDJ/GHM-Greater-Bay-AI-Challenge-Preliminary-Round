...
class LoRALinearLayer(nn.Module):
    def __init__(self, in_features, out_features, rank=24, network_alpha=None, device='cuda:0', dtype=None):
        super().__init__()
        print(in_features,out_features,rank)
...
class lora_cross_attention(nn.Module):
...
class ModelWithAdapter(nn.Module):
    def __init__(self, pretrained_attention_model, adapter):
        super(ModelWithAdapter, self).__init__()
        self.pretrained_attention_model = pretrained_attention_model
        self.adapter = adapter

    def forward(self, input_data):
        attention_output = self.pretrained_attention_model(input_data)
        adapter_output = self.adapter(attention_output)
        final_output = attention_output + adapter_output
        return final_output
...
class UViT(nn.Module):
    ...
    def forward(self, img, clip_img, text, t_img, t_text, data_type):
        ...
        for blk in self.in_blocks:
            ...
            model = lora_cross_attention()
            model.to('cuda')
            lora_img = model(img, text)
            x = torch.cat((t_img_token, t_text_token, token_embed, text, clip_img, img), dim=1)
